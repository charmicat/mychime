...
public class MainActivity extends AppCompatActivity implements MyPreferences.OnConfigurationChangedListener {
    ...
    public void onStop() {
        ...
        if (!isServiceRunning) {
            Log.d(TAG, "Service is not running");
            Toast toast = Toast.makeText(this, getResources().getString(R.string.serviceStopped), Toast.LENGTH_LONG);

            toast.show();
        } else {
            Log.d(TAG, "Service is running");
            Toast toast = Toast.makeText(getApplication(), getResources().getString(R.string.serviceRunning),
                    Toast.LENGTH_LONG);
            toast.show();
        }
    }
